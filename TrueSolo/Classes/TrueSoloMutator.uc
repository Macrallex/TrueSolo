...
// DAMAGE SCALING FOR ZEDS BELOW | The Killing Floor 2 wiki's inidividual Zed pages were also used as reference for damage and movespeed scale values. More info in the .ini file.
	...
	function ModifyAI(Pawn AIPawn) 
	{
		if(KFPawn_ZedBloat(AIPawn)!=None) 
		{
			...
			KFPawn_Monster(AIPawn).InitialGroundSpeedModifier = BloatMoveSpeedScale; 			// BLOAT MOVESPEED SCALING <--- Modifies the "InitialGroundSpeedModifier" variable for particular Zed located in KFPawn_Monster within C:\Program Files (x86)\Steam\steamapps\common\killingfloor2\Development\Src\KFGame\Classes
		}
		...
		if(KFPawn_ZedBloatKing(AIPawn)!=None) 
		{
			...
			KFPawn_Monster(AIPawn).InitialGroundSpeedModifier = AbominationMoveSpeedScale; 		// ABOMINATION MOVESPEED SCALING 
		}
		...
		if(KFPawn_ZedFleshpoundKing(AIPawn)!=None) 
		{
			...
			KFPawn_Monster(AIPawn).InitialGroundSpeedModifier = KingFleshpoundMoveSpeedScale; 	// KING FLESHPOUND MOVESPEED SCALING 
		}
		...
		if(KFPawn_ZedHans(AIPawn)!=None) 
		{
			...
			KFPawn_Monster(AIPawn).InitialGroundSpeedModifier = HansMoveSpeedScale; 			// HANS VOLTER MOVESPEED SCALING 
		}
		...
		if(KFPawn_ZedMatriarch(AIPawn)!=None) 
		{
			...
			KFPawn_Monster(AIPawn).InitialGroundSpeedModifier = MatriarchMoveSpeedScale; 		// MATRIARCH MOVESPEED SCALING 
		}
		...
		if(KFPawn_ZedPatriarch(AIPawn)!=None) 
		{
			...
			KFPawn_Monster(AIPawn).InitialGroundSpeedModifier = PatriarchMoveSpeedScale;        // PATRIARCH MOVESPEED SCALING 
		}
