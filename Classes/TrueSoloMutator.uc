...
// I - GUIDE TO THE CODE - A relatively brief guide that hopefully helps you make sense of my source code in general.
	...
	// II - WHAT IS THIS? - What is this guide and why is here?
		...
		// If you are not already, I recommend using Notepad++ or similar software for ease in reading the source files!
		...
		// try and make some other developer's life easier like mine was through reading my source code so I've taken special care to leave an abundant presence of comments throughout to help make sense of things in addition 
		// to this little guide. Despite the line count and file size of this mutator, it actually doesn't touch on as much in the game as you might think. It only modifies and lets users further modify Zed damage, Zed movement 
		// speed, Syringe base recharge, and Syringe base healing. In fact, most of the reason why this mutator is as large as it is is purely for the purpose of allowing a substantial user customization. Unfortunately, 
		// due the nature of this mutator, its source code isn't going to guide you very much for other topics like weapon modification, skin creation, custom 3D models, etc., but what you read and analyze here can 
		// very well be a decent stepping stone to learning to practice the more advanced topics. This guide is a TL:DR of the source code in general and the comments located through this file are intended to assist users 
		// utilize the INI after understanding the inner workings better. For other developers, the comment structure aids in my source code's teachability and additionally eases the debugging process for any issues found in
		// the future by me and/or others.
		 
	
	// II - WHERE AM I AND WHAT ARE ALL THESE DOUBLE SLASHES? - What's this file and what are these non-code portions?
	
		// This is the mutator's main class file and is critical to its general function. The "//"s in the file signify comment code I left throughout for explicitly for explanatory purposes primarily to assist other developers, 
		// although they can also be utilized by users to have an easier time customizing the INI through understanding better what's happening behind the scenes. Furthermore, these comments will also ease issue identification
		// and resolution by providing more understandable explanations of potentially problematic code. 
	...
	// II - THE SOURCE FILES - A list of the mutator's more useful uncompiled files and their explanations. You can find these files directly here: https://github.com/Macrallex/TrueSolo. Please suffix file URLs with "?ts=4" 
		...
		// III - MAIN FOLDER - Labelled "True Solo". This is the main folder of the mutator and holds every part of it within its sub-folders. "TrueSolo.TrueSoloMutator" is what the game recognizes the mutator by via console.
			...
			// When the KF2 UDK is downloaded you will have access to the game's source code. The vanilla files I extended from and reference below can be found there in KF2's development folder within steamapps.
		
		
		
			// IV - CLASS FOLDER - The "Class" folder. Stores and identifies to the game my custom extended child classes. The meat of the mutator and where all the important stuff happens. 
				...
				// V - TrueSoloMutator.uc - The main mutator file extending off the vanilla "KFMutator.uc". A necessary part of all mutators. 
					...
					// also called to replace the vanilla Syringe here. The core of the mutator's functioning.
				...
				// V - TrueSoloHealingBase.uc - The file extends off the vanilla "KFWeap_HealerBase.uc". It is where I create the healing behavior for my custom Syringe.
					...
					// The third largest in file size, this file is where customizable scales for my custom Syringe are defined and logically assigned.  
			...
			// IV - CONFIG FOLDER - The "Config" folder. Stores and identifies to the game my custom INI. The game pulls values from it to assign my variables according to the logic I've defined in my UCs.
	...
	// II - TABLE OF CONTENTS - A basic location guide to the contents of this file by all tier I categories. I again recommend using Notepad++ for this to make sense although other code editing programs are available.
	
		// III - GUIDE TO THE CODE - Lines 8 to 135
		
			// Part of what you are reading right now. An explanatory guide that hopefully assists overall navigation and comprehension of my source code.
			
			
		// III - VARIABLE DECLARATIONS - Lines 142 to 1188
			...
			// All of these variables are what're called Config variables. Every "config" variable can be tailored in KFTrueSolo.ini to your personal tastes. These variable listings are comprised of damage and movement speed
			// scales for Zeds. Within these two categories are individual and blanket scales per difficulty and special toggleable variable sets: Default, Universal, Player-Scaled Damage, and Player-Scaled Movement Speed.
		...
		// III - MAKE SURE THE MUTATOR IS RUNNING OR SOMETHING - Lines 1196 to 1228
		
			// A small but crucial section. These functions ensure the mutator is properly loaded before performing its functions. 
		
		
		// III - REPLACING THE SYRINGE - Lines 1229 to 1272
	
			// Another minor but very important section. Deletes the vanilla Syringe and replaces its inventory slot with the custom, customizable one I created with the other 3 UC files.
		

		// III - ZED DAMAGE AND MOVEMENT SPEED, SPECIAL VALUE SETS - Lines 1273 to 2314

			// Previously declared variables for Zed damage and movement are appropriately sorted into four special value replacement sets, each toggleable and customizable to the user in the INI:  
			//
			// 1. Scaling Zed damage blankets by player count up to 6+
			// 2. Scaling Zed movement speed blankets by player count up to 6+
			// 3. A universal value set that is active regardless of difficulty or player count
			// 4. A default value set that allows enabling and disabling the mutator's default settings while preserving user customization.
			//
			...
			// Depending on the user's customization choices in the INI, certain portions of the active by default (note: NOT the Default value set which is its own set) variable set will have their values replaced with that 
			// of the requested special variable sets. If you're wondering what exactly "blankets" are, they're multipliers that are logically applied over the individual scales of every Zed for a certain difficulty 
			// and/or player count depending on the customization choices the user made. Say you want to multiply the current individual damage scales of all Zeds across the board on Normal by 1.34x with one value change.  
			// You can do that via the blankets and similar changes can also be made to Zed movement speed. Make blanket changes to Zeds with minimal effort! This concept is explained more in the INI.
			
		
		// III - ZED SCALE ASSIGNMENTS - Line 2315 to 2999

			// After the logical parameters in the previous section have been evaluated based off what was read in the INI, the appropriate scales are Zeds' damage and movement speed are finally assigned to them here.
			// At the end of the document is a single entry in "defaultproperties" to assign a class path for my Syringe. This is called upon by the "Replacing the Syringe" section. 
			

// I - VARIABLE DECLARATIONS - Creating variables with their names and types for later usage.

	// II - SYRINGE - Variable for my custom Syringe.
	
		var private const class<KFWeapon> TrueSoloHealingSyringe; 			// Creates a variable name for my custom syringe to be used in replacing the original syringe.
	...
	// II - CONVENIENCE VARIABLES - These variables make customization easier by allowing sweeping scale changes with minimal INI tweaking.
		...
		// III - BLANKET SCALING VARIABLES - These variables affect either the movement speed or damage for every Zed over a chosen difficulty.
			...
			// IV - BLANKET ZED DAMAGE SCALING VARIABLES - Scales that are multiplied over all of the Zed damage scales for the selected difficulty. Useful for easily making blanket changes with one value change. 
				...
				// V - PLAYER COUNT CUSTOMISATION VARIABLES - If users toggle player scaling for damage, damage blankets over a particular difficulty will equal these depending on player count
					...
					// VI - 1 PLAYER - The damage of every Zed across the chosen difficulty will be further multiplied by this much when one player is present.
					...
					// VI - 2 PLAYERS - The damage of every Zed across the chosen difficulty will be further multiplied by this much when two players are present.
					...
					// VI - 3 PLAYERS - The damage of every Zed across the chosen difficulty will be further multiplied by this much when three players are present.
					...
					// VI - 4 PLAYERS - The damage of every Zed across the chosen difficulty will be further multiplied by this much when four players are present.
					...
					// VI - 5 PLAYERS - The damage of every Zed across the chosen difficulty will be further multiplied by this much when five players are present.
					...
					// VI - 6 PLAYERS AND UP - The damage of every Zed across the chosen difficulty will be further multiplied by this much when six or more players are present.
			...
			// IV - BLANKET ZED MOVESPEED SCALING VARIABLES - Scales that are multiplied over all of the Zed move speed scales for the selected difficulty. Useful for easily making blanket changes with one value change. 
				...
				// V - PLAYER COUNT CUSTOMISATION VARIABLES - The above move speed scales will equal these depending on player count.
					...
					// VI - 1 PLAYER - The move speed of every Zed across the chosen difficulty will be further multiplied by this much when one player is present.
					...
					// VI - 2 PLAYERS - The move speed of every Zed across the chosen difficulty will be further multiplied by this much when two players are present.
					...
					// VI - 3 PLAYERS - The move speed of every Zed across the chosen difficulty will be further multiplied by this much when three players are present.
					...
					// VI - 4 PLAYERS - The move speed of every Zed across the chosen difficulty will be further multiplied by this much when four players are present.
					...
					// VI - 5 PLAYERS - The move speed of every Zed across the chosen difficulty will be further multiplied by this much when five players are present.
					...
					// VI - 6 PLAYERS AND UP - The move speed of every Zed across the chosen difficulty will be further multiplied by this much when six players or more are present.
		...
		// III - DEFAULT VARIABLES - These variables store the mutator's default settings. 
			...
			// IV - BOOLEAN - Decides through true/false whether default settings are to be activated or not. Useful for when you want to switch in and out from your customizations without value or INI replacement.
				...
				var config bool UseDefault; // True/false boolean for the Default set
			...
			// IV - BLANKETS, DEFAULT - These blanket variables for Zeds will be activated if default settings are toggled.
				...
				// V - ZED DAMAGE, DEFAULT - Default Zed damage blankets per difficulty.
					...
					var config float DamageScaleNorDef;					// DEFAULT NORMAL			- The default amount every Zed's damage scale is further multiplied by on Normal, which is 1.
					var config float DamageScaleHarDef;					// DEFAULT HARD				- The default amount every Zed's damage scale is further multiplied by on Hard, which is 1.
					var config float DamageScaleSuiDef;					// DEFAULT SUICIDAL         - The default amount every Zed's damage scale is further multiplied by on Suicidal, which is 1.
					var config float DamageScaleHelDef;					// DEFAULT HELL ON EARTH    - The default amount every Zed's damage scale is further multiplied by on Hell on Earth, which is 1.
				
				// V - ZED MOVE SPEED, DEFAULT - Default Zed move speed blankets per difficulty. Multiply every 
				
					var config float MoveSpeedScaleNorDef;				// DEFAULT NORMAL			- The default amount every Zed's damage scale is further multiplied by on Normal, which is 1.
					var config float MoveSpeedScaleHarDef;              // DEFAULT HARD				- The default amount every Zed's damage scale is further multiplied by on Hard, which is 1.
					var config float MoveSpeedScaleSuiDef;              // DEFAULT SUICIDAL         - The default amount every Zed's damage scale is further multiplied by on Suicidal, which is 1.
					var config float MoveSpeedScaleHelDef;              // DEFAULT HELL ON EARTH    - The default amount every Zed's damage scale is further multiplied by on Hell on Earth, which is 1.
				
		
		
			// IV - INDIVIDUAL SCALES, DEFAULT - Default scales for each Zed type.
	...
	// II - DIFFICULTY SPECIFIC DAMAGE SCALING VARIABLES - Individual damage scales for every Zed for each difficulty. These variables are active by default unless a special value set is toggled to override them.
	...
	// II - DIFFICULTY SPECIFIC ZED MOVESPEED SCALING VARIABLES - Individual movement speed scales for every Zed for each difficulty. These variables are active by default unless a special value set is toggled to override them.
		...
		// III - ZED MOVESPEED SCALING VARIABLES - SUICIDAL - These individual movement speed scales for Zeds will be active by default assuming no special value sets are toggled. The numbers are for quality of life 
			...
			// IV - BOSS ZEDS, SUICIDAL 					5 - 5/24 							The individual Suicidal movement speed scales for boss class Zeds.
			...
			// IV - LARGE ZEDS, SUICIDAL 					3 - 8/24                          The individual Suicidal movement speed scales for large class Zeds.       
			...
			// IV - MEDIUM TRASH ZEDS, SUICIDAL 			6 - 14/24                         The individual Suicidal movement speed scales for medium trash class Zeds.
			...
			// IV - ELITE TRASH ZEDS, SUICIDAL  			3 - 17/24                         The individual Suicidal movement speed scales for elite trash class Zeds. 
			...
			// IV - WEAK TRASH ZEDS, SUICIDAL  				7 -  24/24                        The individual Suicidal movement speed scales for weak trash class Zeds.  
...
// I - REPLACING THE SYRINGE - Replacing the vanilla Syringe with my custom, customizable Syringe that I created using my other 3 UC files. Re-purposed code from 
	...
	// II - SYRINGE REPLACEMENT FUNCTION - The process of replacing the Syringe
		...
		private final function ReplaceSyringe(Pawn P)
		{
			...
			if (KFIM != none) 					// If the inventory manager is active, proceed to the logic below.
			{
				...
				} 																				// If the real solo syringe is available, then create it in the player's inventory
...
// I - ZED DAMAGE AND MOVEMENT SPEED, SPECIAL VALUE SETS - These value sets override the active by default variables should the user toggle their activation in the mutator's INI.
	...
	// II - BLANKET SCALE PLAYER COUNT BOOLEANS - Does the user want blanket scales to automatically scale with player count according to customizable amounts?
		...
		// III - DAMAGE SCALES BOOLEAN - Does the user want Zed damage blanket scales to automatically scale with player count according to customizable amounts?
			...
			{
				if(WorldInfo.Game.NumPlayers == 1)						// If the player count is 1, further multiply every Zed's individual damage scale on the chosen difficulty by these amounts.
				{
				...
				else if(WorldInfo.Game.NumPlayers == 2)					// If the player count is 2, further multiply every Zed's individual damage scale on the chosen difficulty by these amounts.
				{
				...
				else if(WorldInfo.Game.NumPlayers == 3)					// If the player count is 3, further multiply every Zed's individual damage scale on the chosen difficulty by these amounts.
				{
				...
				else if(WorldInfo.Game.NumPlayers == 4)					// If the player count is 4, further multiply every Zed's individual damage scale on the chosen difficulty by these amounts.
				{
				...
				else if(WorldInfo.Game.NumPlayers == 5)					// If the player count is 5, further multiply every Zed's individual damage scale on the chosen difficulty by these amounts.
				{
				...
				else if(WorldInfo.Game.NumPlayers >= 6)					// If the player count is 6 or more, further multiply every Zed's individual damage scale on the chosen difficulty by these amounts.
				{
		...
		// III - Move Speed Scales Boolean - Does the user want Zed movement speed blanket scales to automatically scale with player count according to customizable amounts?
			...
			{
				if(WorldInfo.Game.NumPlayers == 1)						// If the player count is 1, further multiply every Zed's invididual movement speed scale on the chosen difficulty by these amounts.
					{
					...
					else if(WorldInfo.Game.NumPlayers == 2)				// If the player count is 2, further multiply every Zed's invididual movement speed scale on the chosen difficulty by these amounts.
					{
					...
					else if(WorldInfo.Game.NumPlayers == 3)				// If the player count is 3, further multiply every Zed's invididual movement speed scale on the chosen difficulty by these amounts.
					{
					...
					else if(WorldInfo.Game.NumPlayers == 4)				// If the player count is 4, further multiply every Zed's invididual movement speed scale on the chosen difficulty by these amounts.
					{
					...
					else if(WorldInfo.Game.NumPlayers == 5)				// If the player count is 5, further multiply every Zed's invididual movement speed scale on the chosen difficulty by these amounts.
					{
					...
					else if(WorldInfo.Game.NumPlayers >= 6)				// If the player count is 6 or more, further multiply every Zed's invididual movement speed scale on the chosen difficulty by these amounts.
					{
	...
	// II - DEFAULT VARIABLES BOOLEAN - Does the user want to enable to mutator's default settings? Note that this variable set is different than the one labeled as active by default.
		...
		// III - DEFAULTING - Overriding special value sets and user customizations for as long as default settings are enabled.
			...
			// IV - INDIVIDUAL ZED DAMAGE SCALE REPLACEMENT, DEFAULT - Assign all individual Zed damage scales the values of their default versions. These individual damage scales for Zeds will be active if default 
				...
				// V - BOSS ZEDS, DEFAULT 						 					5 - 5/24 				Replacing the individual damage scales for boss class Zeds with their default versions.
				...
				// V - LARGE ZEDS, DEFAULT 											3 - 8/24				Replacing the individual damage scales for large class Zeds with their default versions.				
				...
				// V - MEDIUM TRASH ZEDS, DEFAULT 									 6 - 14/24		 		Replacing the individual damage scales for medium trash class Zeds with their default versions.
				...
				// V - ELITE TRASH ZEDS, DEFAULT 									 3 - 17/24				Replacing the individual damage scales for elite trash class Zeds with their default versions.
...
// I - ZED SCALE ASSIGNMENTS - After evaluating the logic in the special value sets section, let's apply the right scales to the Zeds. Multiple references used for accurate 1:1 damage and movement speed scale values. 
	...
	// II - BOSS ZEDS 													 5 - 5/24 							Assigning the proper scales for boss class Zeds
		...
		// III - Matriarch - 3
			...
			{																										// NOTE: 
																													...
																													// are 1.0 with the exception of Hans, Patty, and Matty of course as I set theirs to 
																													// 1.3 to replicate co-op's Last Man Standing after 2 minutes.
		...
		// III - Hans - 4
			...
			if(KFPawn_ZedHans(AIPawn)!=None) 																		// If Hans exists, apply the appropriate following scales to him.
			{
				...
				else																									// If the difficulty is Hell on Earth, use Hans' damage and movement scales for Hell on Earth.
				{
		...
		// III - Patriarch - 5
			...
			if(KFPawn_ZedPatriarch(AIPawn)!=None) 																	// If the Patriarch exists, apply the appropriate following scales to him.
			{
				...
				else																									// If the difficulty is Hell on Earth, use the Patriarch's damage and movement scales for Hell on Earth.
				{
	...
	// II - ELITE TRASH ZEDS 			 								3 - 17/24 							Assigning the proper scales for elite trash class Zeds
		...
		// III - ELITE CRAWLER - 2, damage variables don't affect gas.
