...
// I - GUIDE TO THE FILE - A basic explanatory and navigational guide to this UC file. If you were looking for a more expansive guide over this mutator's source code as a whole, please look at the main mutator file instead.
	...
	// II - WHAT IS THIS? - What is this guide and why is it here?
		...
		// Although this file is pretty small in comparison to the INI and main mutator file, having a basic explanatory and navigational guide for it certainly helps for improving overall mutator usability, debugging, and understanding.
	...
	// II - ORGANIZATIONAL SHCEME - What format do I use to organize things here?
		...
		// III - CODE IS ORGANIZED ACCORDING TO ROMAN NUMERALS - See the link if you don't know what these are: https://www.unrv.com/culture/roman-numerals.php 
			...
			// separation. If you are unsure of where you are in the file, pay attention the category names and numeral counts to trace them back to their parent categories.
			
	
	// II - TABLE OF CONTENTS - Navigational aid to the contents of this file along with their summaries.

		// III - GUIDE TO THE FILE  	Lines 6 - 28
		
			// An explanatory guide to this file as a whole. Serves a variety of purposes both for users and developers, me included. Intended to improve the experience surrounding the understanding, usage, and debugging of the mutator.
			
			
		// III - SYRINGE VARIABLES		Lines 29 - 401
		
			// Where all the the variables that the Syringe can call values from depending on the user's desires are created and neatly organized. All of these values can be customized by the user in the INI that's downloaded with the mutator.
			// I'd also like to note here about how blanket variables work for the Syringe specifically. Let's say you have these values in the INI within the Syringe's Primary Customization Section for Hell on Earth:
			// 
			//
			...
			// 
			//
			// Let's say that you want to change both of these values for Hell on Earth but by the same amount proportionally, for example you want the Syringe's healing amount in general to be twice as much. You look for the Syringe's base healing blanket
			// for Hell on Earth and assign to it "2.0". Now let's look at how that affects the two values above:
			//
			//
			...
			//
			//
			// The Syringe now effectively heals twice the amount at base for yourself in addition to when healing others with one value change. This same process can be applied if users wish to modify the Syringe's base recharge time instead.
			
			
		// III - SYRINGE FUNCTIONALITY	Lines 403 - 799
...
// I - SYRINGE VARIABLES - Where all the Syringe's used variables are created to be assigned later.
	...
	// II - CONVENIENCE VARIABLES - The variables for the Default, Universal, Player Scaling Base Healing Amount, and Player Scaling Base Recharge Time toggleable special variable sets. The primary and active by default blanket variables are included too.
		...
		// III - PLAYER COUNT SCALES - The blanket variables to be logically used if the user wants one or both of the player scaling options for the Syringe in play.
			...
			// IV - THREE PLAYERS - The blanket variables to come in to play when the player count is one on a per-difficulty basis.
			...
			// IV - FOUR PLAYERS - The blanket variables to come in to play when the player count is one on a per-difficulty basis.
			...
			// IV - FIVE PLAYERS - The blanket variables to come in to play when the player count is one on a per-difficulty basis.
			...
			// IV - SIX OR MORE PLAYERS - The blanket variables to come in to play when the player count is one on a per-difficulty basis.
		...
		// III - UNIVERSAL SCALES - Should the user decide to enable the Universal special variable set, these blanket variables for the Syringe will be in play regardless of difficulty or player count.
			...
			// IV - BLANKETS, UNIVERSAL - The universal blanket variables for the Syringe's general base healing and recharge time. Multiplied over the individual universal Syringe variables below.
...
// I - SYRINGE FUNCTIONALITY - Defining the logic paths for which the declared variables above are to come into play under.
	...
	// II - VARIABLE REPLACEMENT CHECK - Check what the user has the special value set booleans as and replace variables appropriately.
		...
		// III - PLAYER SCALED HEALING REPLACEMENT - Replacing the appropriate general base healing and recharge time blankets with their player-scaled versions depending on whether one of or both player scaling sets for the Syringe were enabled.
			...
			// IV - VARIABLE REPLACEMENT, DEFAULT - Replacing the appropriate Syringe variables with their default versions.
				...
				// V - BLANKETS, DEFAULT - Replacing the Syringe's blanket variables with their default versions.
					...
					// VI - BASE RECHARGE TIME AFTER HEALING YOURSELF AND OTHERS - Replacing the variables for the Syringe's base recharge time.
						...
						SyringeRechargeScaleNor = SyringeRechargeScaleNorDef;		// Assigning the value of the Syringe's Normal base healing blanket with the default base recharge blanket.
						SyringeRechargeScaleHar = SyringeRechargeScaleHarDef;	    // Assigning the value of the Syringe's Hard base healing blanket with the default base recharge blanket.
						SyringeRechargeScaleSui = SyringeRechargeScaleSuiDef;       // Assigning the value of the Syringe's Suicidal base healing blanket with the default base recharge blanket.
						SyringeRechargeScaleHel = SyringeRechargeScaleHelDef;       // Assigning the value of the Syringe's Hell on Earth base healing blanket with the default base recharge blanket.
				...
				// V - INDIVIDUAL SCALES BY DIFFICULTY, DEFAULT - Replacing the Syringe's difficulty-specific individual base healing and recharge time scales with their default versions.
					...
					// VI - RECHARGE AFTER HEALING YOURSELF AND OTHERS, DEFAULT - Replacing the Syringe's base recharge time scales with their default versions.
						...
						// VII - SUICIDAL SYRINGE - The Syringe's base recharge time on Suicidal.
							...
							SyringeRechargeSui = SyringeHealAmountSuiDef;    				// Suicidal base healing amount
							SyringeOthersRechargeSui = SyringeOthersHealAmountSuiDef;		// Suicidal base healing amount when healing others
							
						// VII - HARD SYRINGE - The Syringe's base recharge time on Hard.
					
							SyringeRechargeHar = SyringeHealAmountHarDef;   				// Hard base healing amount
							SyringeOthersRechargeHar = SyringeOthersHealAmountHarDef;		// Hard base healing amount when healing others
							
						// VII - NORMAL SYRINGE - The Syringe's base recharge time on Normal.
						
							SyringeRechargeNor = SyringeHealAmountNorDef;     				// Normal base healing amount
							SyringeOthersRechargeNor = SyringeOthersHealAmountNorDef;		// Normal base healing amount when healing others
			}
	...
	// II - APPLYING PLACEHOLDER VARIABLES - Although the logic for replacing the placeholder Syringe variables is now already laid out, applying the placeholder variables to the Syringe still must be done.
		...
		{
				if( CurrentFireMode == DEFAULT_FIREMODE )	// If you are healing someone else, apply the primary variables I declared and go through the logic outlined for them to decide if they are to be replaced further with special variable sets.
				{
				...
				else if( CurrentFireMode == ALTFIRE_FIREMODE )	// If you are healing yourself, apply the primary variables I declared and go through the logic outlined for them to decide if they are to be replaced further with special variable sets.
				{
